	...
	*/
	...
	Ext.ux.swfbtn = function(cfg){
    	...
    	var fileprogressbar = new Ext.ProgressBar({
    		text: Application.i18n.getMsg( 'ext.plugins.swfbtn.fup' ),
    		hidden: true,
    		height: 25,
    		autoWidth: true
    	});
    	...
    	var o = new Ext.ProgressBar({
    		text: Application.i18n.getMsg( 'ext.plugins.swfbtn.op' ),
    		height: 25,
    		hidden: true,
    		autoWidth: true
    	});
		...
		function translateSwfStatus(val, meta, rec, row, col, store){
			switch(val){
				case -1 :
					text = Application.i18n.getMsg( 'ext.plugins.swfbtn.queued' );
				break;
				case -2:
					text = Application.i18n.getMsg( 'ext.plugins.swfbtn.uploading' );
				break;
				case -3:
					text = Application.i18n.getMsg( 'ext.plugins.swfbtn.erro' );
				break;
				case -4:
					text = Application.i18n.getMsg( 'ext.plugins.swfbtn.complete' );
				break;
				default:
					text = val;
				break;
		...
		var cm = new Ext.grid.ColumnModel({
			...
			columns: [
				{header: Application.i18n.getMsg( 'ext.plugins.swfbtn.nome' ), width: 150, dataIndex: 'name'},
				{header: Application.i18n.getMsg( 'ext.plugins.swfbtn.tamanho' ), width: 60, dataIndex: 'size'},
				{header: Application.i18n.getMsg( 'ext.plugins.swfbtn.criado' ), width: 300, dataIndex: 'creationdate'},
				{id: 'filestatus', header: Application.i18n.getMsg( 'ext.plugins.swfbtn.status' ), width: 100, dataIndex: 'filestatus', renderer: translateSwfStatus}
			]
		});
		
		var addfiles = new Ext.Button({
			text: cfg.isSingle ? Application.i18n.getMsg( 'ext.plugins.swfbtn.selArquivo' ) : Application.i18n.getMsg( 'ext.plugins.swfbtn.adArquivo' ),
			...
			listeners: {
				'render': function(btn) {
                                ...
                                $.ajax({
                                    ...
                                    dataType: 'json',
                                    cache: false,
                                    url: Application.contextPath + '/ajax/configuracoes/tamanhoMaximoArquivo.jsp',
                                    success: function( data, textStatus ) {
                                        if ( data.success ) {
                                            cfg.filesizelimit = data.valor + ' B';
                                        } else {
                                            cfg.filesizelimit = '512 KB';
                                        }
                                    }
                                });
						...
						try { // Handle this error separately because we don't want to create a FileProgress element for it.                 
							switch (ec) {
		                         ...
		                         case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:                        
		                         	msg = Application.i18n.getMsg( 'ext.plugins.swfbtn.erro.grande' );
		                         	this.debug("Error Code: File too big, File name: " + file.name + ", File size: " + file.size + ", Message: " + message);                         
		                         case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:                         
		                         	msg = Application.i18n.getMsg( 'ext.plugins.swfbtn.erro.vazio' );
		                         	this.debug("Error Code: Zero byte file, File name: " + file.name + ", File size: " + file.size + ", Message: " + message);                         
		                         case SWFUpload.QUEUE_ERROR.INVALID_FILETYPE:                         
		                         	msg = Application.i18n.getMsg( 'ext.plugins.swfbtn.erro.tipo' );
		                         	this.debug("Error Code: Invalid File Type, File name: " + file.name + ", File size: " + file.size + ", Message: " + message);                         
		                         default:
		                         	msg = Application.i18n.getMsg( 'ext.plugins.swfbtn.erro.geral' );
		                         	this.debug("Error Code: " + errorCode + ", File name: " + file.name + ", File size: " + file.size + ", Message: " + message);                         
		                     }         
		                 } catch (e) {         } 	
		                Ext.MessageBox.show({
		                	title: Application.i18n.getMsg( 'ext.plugins.swfbtn.erro.arquivo' ),
		                	msg: (file ? Application.i18n.getMsg('ext.plugins.swfbtn.arquivo')+file.name+msg:cfg.isSingle ? 'Too many files selected.  Max is '+m+', queue cleared.  Please select a single file only.':"Undefined error"),
					...
					function myCustomUploadErrorHandler(file, e, m){
						Ext.MessageBox.show({
							title: Application.i18n.getMsg( 'ext.plugins.swfbtn.erro.enviar' ),
							msg: m,
							buttons: Ext.MessageBox.OK,
							fn: function() {
								clearq.disable();
								if(!cfg.isSingle) {
									o.updateProgress(0, Application.i18n.getMsg( 'ext.plugins.swfbtn.op' ) );
									o.hide();
								}
					...
					function myCustomUploadStartHandler(file){
			    		...
			    		fileprogressbar.updateProgress( 0,
                                                Application.i18n.getMsg( 'ext.plugins.swfbtn.enviando') + file.name + ' (' +
                                                Application.i18n.getMsg( 'ext.plugins.swfbtn.arq') + (swffilessent+1) +
                                                Application.i18n.getMsg( 'ext.plugins.swfbtn.de') + numfiles+')');
				    	return true;
				    }
				    ...
				    function myCustomUploadSuccessHandler(file, server_data, receivedResponse){
				    	...
				    	if(swffilessent == filestore.getCount()) {
				    		Ext.MessageBox.show({
				    			title: Application.i18n.getMsg( 'ext.plugins.swfbtn.upOk' ),
				    			msg: serverMessages,
				    			buttons: Ext.MessageBox.OK,
				    			fn: function() {
				    				filestore.removeAll();
				    ...
				    function myCustomProgressHandler(file, b, t){
				    	...
				    	fileprogressbar.updateProgress( (b/t),
                                                Application.i18n.getMsg( 'ext.plugins.swfbtn.enviando') + file.name + ' (' +
                                                Application.i18n.getMsg( 'ext.plugins.swfbtn.arq') + (swffilessent+1) +
                                                Application.i18n.getMsg( 'ext.plugins.swfbtn.de') + numfiles+') ('+b+'/'+t+')');
		...
		var delfile = new Ext.Button({
			text: cfg.isSingle ? Application.i18n.getMsg( 'ext.plugins.swfbtn.excluirArquivo' ) : Application.i18n.getMsg( 'ext.plugins.swfbtn.excluirDaFila' ),
		...
		var cancelupload = new Ext.Button({
			text: Application.i18n.getMsg( 'ext.plugins.swfbtn.cancelarUp' ),
		...
		var upload = new Ext.Button({
			text: Application.i18n.getMsg( 'ext.plugins.swfbtn.comecarUp' ),
	    		...
	    		if(custompostparams){
	    			...
	    			filestore.each(function(rec){
		    			...
		    			var btn = new Ext.Button({
    						text: Application.i18n.getMsg( 'ext.plugins.swfbtn.salvarDados' ),
    						id: buttonid,
    						listeners: {
    							'click': function(){
	    							filesprocessed += 1;
	    				...
	    				if(!cfg.form) {
		    				var tform = new Ext.form.FormPanel({
		    					...
		    					title: rec.data.name+Application.i18n.getMsg( 'ext.plugins.swfbtn.inforAd' ),
		    					autoWidth: true,
		    					autoHeight: true,
		    					buttons: [btn],
		    					keys: [{
		...
		var clearq = new Ext.Button({
			text: Application.i18n.getMsg( 'ext.plugins.swfbtn.limparFila' ),
			disabled: true,
		...
		var gridwin = new Ext.Window({
			title: Application.i18n.getMsg( 'ext.plugins.swfbtn.titulo' ),
                        iconCls: 'iconeEnviarArquivo',
			closeAction: 'hide',
			autoScroll: true,
			modal: true,
			width: 614,
